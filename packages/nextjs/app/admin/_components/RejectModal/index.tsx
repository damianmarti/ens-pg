...
import { RejectVoteReqBody } from "~~/app/api/stages/[stageId]/reject-vote/route";
import { Button } from "~~/components/pg-ens/Button";
import { FormTextarea } from "~~/components/pg-ens/form-fields/FormTextarea";
import { Address } from "~~/components/scaffold-eth";
import { useFormMethods } from "~~/hooks/pg-ens/useFormMethods";
import { GrantWithStagesAndPrivateNotes } from "~~/types/utils";
...
export const RejectModal = forwardRef<HTMLDialogElement, RejectModalProps>(({ stage, grantName, closeModal }, ref) => {
  ...
  const { mutateAsync: postRejectVote, isPending: isPostingRejectVote } = useMutation({
    mutationFn: (rejectVoteBody: RejectVoteReqBody) =>
      postMutationFetcher(`/api/stages/${stage.id}/reject-vote`, { body: rejectVoteBody }),
  });
